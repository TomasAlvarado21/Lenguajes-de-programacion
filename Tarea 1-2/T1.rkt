...
(define (inlist? String List)
    (match List
        ['() #f]
        [(cons x s) (if (string=? x String) #t (inlist? String s))]))
...
(define (fold-prop fvarp fandp forp fnotp)
    (lambda (p)
        (match p
            [(varp n) (fvarp n)]
            [(andp p q) (fandp ((fold-prop fvarp fandp forp fnotp) p) ((fold-prop fvarp fandp forp fnotp) q))]
            [(orp p q) (forp ((fold-prop fvarp fandp forp fnotp) p) ((fold-prop fvarp fandp forp fnotp) q))]
            [(notp p) (fnotp ((fold-prop fvarp fandp forp fnotp) p))])))



#| Parte B |#
;; Ahora todas las funciones siguientes se pueden definir en base a fold-prop

;; occurrences-2 :: Prop String -> Number
;; funcion que devuelve la cantidad de veces que aparece una variable en una proposicion
(define (occurrences-2 p String)
    ((fold-prop (lambda (n) (if (string=? n String) 1 0))
               (lambda (p q) (+ p q))
               (lambda (p q) (+ p q))
               (lambda (p) p)
               ) p))
...
;; funcion que evalua una proposicion p, obteniendo los valores de cada varaibles desde un ambiente env, devolviendo
(define (eval-2 p env)
    ((fold-prop (lambda (n) (if (assoc n env) (cdr (assoc n env)) ((error (format "eval: variable ~a is not defined in environment" n)))))
               (lambda (p q) (and p q))
               (lambda (p q) (or p q))
               (lambda (p) (not p))
               ) p))
...
(define (simplify-negations-2 p)
    ((fold-prop (lambda (n) (varp n))
               ...
               (lambda (p) 
                     (match p
                          [(varp n) (notp (varp n))]
                          [(andp p q) (orp (simplify-negations-2 (notp p)) (simplify-negations-2 (notp q)))]
                          [(orp p q) (andp (simplify-negations-2 (notp p)) (simplify-negations-2 (notp q)))]
...
(define (distribute-and-2 p)
    ((fold-prop (lambda (n) (varp n))
               (lambda (p q) (match (distribute-and-2 p)
                        [(orp p1 p2) (orp (distribute-and-2 (andp p1 q)) (distribute-and-2 (andp p2 q)))]
                        [p (match (distribute-and-2 q)
                                [(orp q1 q2) (orp (distribute-and-2 (andp p q1)) (distribute-and-2 (andp p q2)))]
                                [q (andp p q)])]))
               (lambda (p q) (orp (distribute-and-2 p) (distribute-and-2 q)))
               (lambda (p) (notp (distribute-and-2 p)))
               ) p))